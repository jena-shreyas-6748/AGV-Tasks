...
#print the total cost of the path and time taken to find the path        
...
def AStar(img,h,w,src_x,src_y,dest_x,dest_y):
    ...
    #create the open point list and the closed point list
    op_list=[]
    cl_list=[]
    ...
    #add starting point object to open list
    heapq.heapify(op_list)
    heapq.heappush(op_list,st_pt)

    while (len(op_list)>0):

        #get the element with smallest 'f' from open list and put it in closed list
        curr=heapq.heappop(op_list)
        cl_list.append(curr)

        img[curr.position[0]][curr.position[1]]=(10,180,240)

        #if destination is found,return the total time taken to find path and return final image with path
...
#check the children; if they are in closed list, skip them
        ...
        for ch in ch_list:
            ...
            for i in range(0,len(op_list)):
                ...
                if (ch.position==op_list[i].position):
                    count=i
                    break

            if count!=0:
                if ch.g>=op_list[count].g:
                    continue

                else:
                    op_list[count]=op_list[-1]
                    op_list.pop()

                    if count<len(op_list):
                        heapq._siftup(op_list,count)
                        heapq._siftdown(op_list,0,count)

            heapq.heappush(op_list,ch)
